        ...
        for i in range(0, up.time_total):
            ...
            for teacher in teachers_subjects:
                if teacher.subject == up.subj and teacher.nclass == up.ncls:
                    app = [up.ncls.pk, up.subj.pk, teacher.teacher.pk]
                    Z.append(app)

                    checkError = False
                    log["OK"].append(f'Получилось: {up.ncls.title, up.subj.title, teacher.teacher.user.username}')
            if checkError:
                log["NO"].append(f'Нет учителя у {up.ncls} для {up.subj}')
                err += 1
    return Z, log, err
...
def mutTT(individual, indpb):
    ...
    for index, item in enumerate(new_lst):
        ...
        if random.random() < indpb:
            ...
            times = avialable_time_v4(individual, individual[count_in_item*(index) + 0])
            individual[z] = random.choice(times)
...
def printTT(timetable):
    ...
    for i in tt:
        ...
        a2 = nClass.objects.get(pk=i[0]) # Класс
        a3 = Subjects.objects.get(pk=i[1]) # Предмет
        a4 = TeacherProfile.objects.get(pk=i[2]) # Учитель
...
def run():
    ...
    POPULATION_SIZE = 2   # количество индивидуумов в популяции
    ...
    if config.get('check_best'):
        ...
        for cond in CONDITIONS:
            i, check = cond(best)
            logger.success(f'check{count} = {check}')
            # text += f'\t check{count} = {check} \n'
            count += 1
            s += i

        # text += f'\t s = {s}'
        logger.success(f's = {s}')
        # print(text)
        # logger.success(text)
        # CONDITIONS = list()
